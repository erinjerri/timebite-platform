...
struct GoalsView: View {
    ...
    private func lifeAreaCard(_ area: String) -> some View {
        let areaGoals = goals.filter { LifeAreaCatalog.normalized($0.lifeArea) == LifeAreaCatalog.normalized(area) }
    ...
    private var selectedLifeAreaDetail: some View {
        let areaGoals = goals.filter { LifeAreaCatalog.normalized($0.lifeArea) == LifeAreaCatalog.normalized(selectedLifeArea) }
    ...
    private func complete(_ goal: Goal) {
        ...
        withAnimation(.spring(response: 0.32, dampingFraction: 0.88)) {
            ...
            try? syncCoordinator.enqueueGoal(goal, baseUpdatedAt: baseUpdatedAt)
        }
    }
